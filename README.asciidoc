...
Celos installation in a central, SFTP-accessible location, and storing
the target file's SFTP URL in an environment variable,
e.g. `TARGET_FILE`.  Deploy scripts using Celos CI should then pass
this variable as the `--target` argument to Celos CI, making them
independent of the Celos installation.  See link:samples/quickstart[]
for an example.
