...
=== celos-server.jar command-line arguments
...
The slot's state will be reset to <<WAITING>> and its retry count will
be reset to 0.
...
== Celos CI Reference
=== celos-ci.jar Command-line Arguments
==== --mode
==== --deployDir
==== --hdfsRoot
==== --target
=== Celos CI Environment Variables
==== CELOS_CI_USERNAME
=== Target files
== Celos UI Reference
=== celos-ui.jar Command-line Arguments
==== --port
==== --celos
==== --hue
==== --config
