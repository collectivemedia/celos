...
public class FixDirTreeConverterTest {
    ...
    public void testFixDirTreeConverter() throws Exception {
        ...
        FixDir expected = createOutDirWithFileContent("lowercase".toUpperCase());

        FixDir transformed = converter.transform(dir, new ReverseStringFixFileConverter());

        FixObjectCompareResult result = new RecursiveDirComparer(expected).check(transformed);

        Assert.assertEquals(result.getStatus(), FixObjectCompareResult.Status.SUCCESS);
    }

    private FixDir createOutDirWithFileContent(String fileContent) {
        FixDir dir1 = createOutDirWithSubdirsAndFile(fileContent);
