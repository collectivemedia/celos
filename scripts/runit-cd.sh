...
#ansible all -i "${DEPLOY_HOST}," -u "${CELOS_USER}" -m command -a "mkdir -p ${DEST_PATH}"
#ansible all -i "${DEPLOY_HOST}," -u "${CELOS_USER}" -m copy -a "src=${JAR_PATH} dest=${DEST_PATH}"
#ansible all -i "${DEPLOY_HOST}," -u "${CELOS_USER}" -m command -a "sv restart ${SERVICE_NAME}"
