...
if [ -z "$(sudo docker ps | grep 'celosnn ')" ]; then
    ...
    chmod 600 scripts/test_rsa
    ssh-add scripts/test_rsa

    rm -f hoststempfile
    scp -o StrictHostKeyChecking=no root@$IP_ADDRESS:/etc/hosts scripts/hoststempfile
    scp -o StrictHostKeyChecking=no scripts/hoststempfile root@$IP_ADDRESS1:/etc/hosts
    scp -o StrictHostKeyChecking=no scripts/hoststempfile root@$IP_ADDRESS2:/etc/hosts
