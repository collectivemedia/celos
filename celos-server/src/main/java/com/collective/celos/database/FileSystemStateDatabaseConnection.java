 ...
 */
public class FileSystemStateDatabaseConnection implements StateDatabaseConnection {

    public static class Config implements StateDatabaseConfig {
    ...
    private static final Logger LOGGER = Logger.getLogger(FileSystemStateDatabaseConnection.class);
     ...
     */
    public FileSystemStateDatabaseConnection(Config config) throws IOException {
        if (!Util.requireNonNull(config.dir).exists()) {
            throw new IOException("Database directory " + config.dir + " doesn't exist.");
        }
        stateDir = new File(config.dir, STATE_DIR_NAME);
        rerunDir = new File(config.dir, RERUN_DIR_NAME);
        pausedDir = new File(config.dir, PAUSED_DIR_NAME);
        registerDir = new File(config.dir, REGISTER_DIR_NAME);
    }
